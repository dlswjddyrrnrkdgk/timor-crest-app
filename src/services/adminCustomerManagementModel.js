...
export function buildCustomerManagementSummary(data = {}, now = new Date()) {
  ...
  return {
    kpis: {
      ...
      consultationsThisMonth: consultations.thisMonth,
      upcomingMeetings: events.upcoming.length,
...
export function calculateLeadStats(leads = []) {
  ...
  return {
    ...EMPTY_STATS,
    count: rows.length,
    recent: [...rows].sort(compareRecent).slice(0, 5),
  };
}
...
export function calculateConsultationStats(notes = [], now = new Date()) {
  ...
  return {
    ...EMPTY_STATS,
    count: rows.length,
    thisMonth: rows.filter((note) => isSameMonth(note?.consultation_date, current)).length,
    recent: [...rows].sort(compareRecent).slice(0, 5),
  };
}
...
export function calculateEventStats(events = [], now = new Date()) {
  ...
  const upcoming = rows
    ...
    .filter((event) => isOnOrAfterDate(event?.event_date, current))
    .sort(compareEventDate);

  return {
    ...EMPTY_STATS,
    count: rows.length,
    today: upcoming.filter((event) => isSameDay(event?.event_date, current)),
    upcoming,
    recent: [...rows].sort(compareEventDate).slice(0, 5),
  };
}
...
function isOnOrAfterDate(value, current) {
  ...
  const currentOnly = new Date(current.getFullYear(), current.getMonth(), current.getDate()).getTime();
  return dateOnly >= currentOnly;
}

function compareRecent(left, right) {
  return (toDate(right?.lead_date || right?.report_date || right?.consultation_date || right?.created_at)?.getTime() || 0)
    - (toDate(left?.lead_date || left?.report_date || left?.consultation_date || left?.created_at)?.getTime() || 0);
}

function compareEventDate(left, right) {
  return (toDate(`${right?.event_date || ""}T${right?.start_time || "00:00:00"}`)?.getTime() || 0)
    - (toDate(`${left?.event_date || ""}T${left?.start_time || "00:00:00"}`)?.getTime() || 0);
}
